...
def _extract_main_number(x: str) -> float:
    if pd.isna(x):
        return float('nan')
    s = str(x)
    s = s.split('±', 1)[0]  # fast path
    m = re.search(r'[-+]?\\d*\\.?\\d+(?:[eE][-+]?\\d+)?', s)
    return round(float(m.group(0)), 3) if m else float('nan')
...
features=pd.read_csv(f"{results_folder}/{name_dataset}_all_oddsratio_summaries_concatenated.csv", index_col=None)
m = features['Dataset'].str.extract(r'^(?P<p0>[^_]+)(?:_(?P<p1>[^_]+))?(?:_(?P<rest>.*))?$')
features['Species'] = m['p0'].where(m['p1'].isna(), m['p0'] + '_' + m['p1'])
features['Antibiotic']  = m['rest']

features=features[features['OR_95CI_Lower']>1]
features=features[features['Odds_Ratio']>odds_ratio_threshold]
features = features[features['Dataset'].isin(selected_models.index)]

features = pd.merge(features, classes, left_on='Antibiotic', right_on='Antibiotic Name', how='inner')
...
directory = Path('./')
file=f"Forecasting/LinearRegression/LinearRegression_{name_dataset}_permutation.xlsx"
newfile=f"Forecasting/LinearRegression/LinearRegression_{name_dataset}_permutation_MLselectedFDRcorrected.xlsx"
...
linear_df = linear_df[linear_df["significant"]==True]
...
forecasting=pd.read_csv(f"Forecasting Global/Trend_Analysis.csv")


forecasting=forecasting[forecasting['Specie']==name_dataset]
forecasting=forecasting[["Specie", "Antibiotic Class", "Indicator", "Feature", "Pearson r", "Pearson p-value", "Countries", "Num Countries", "Years", "Num Years","First Year", "Last Year", "Feature ADF Test statistic", "Feature ADF p-value", "Feature KPSS Test statistic", "Feature KPSS p-value","Feature Result 95%", "Feature Trend 95%",	"Indicator ADF Test statistic", "Indicator ADF p-value", "Indicator KPSS Test statistic","Indicator KPSS p-value", "Indicator Result 95%", "Indicator Trend 95%"]]
