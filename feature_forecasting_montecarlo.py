...
if __name__ == "__main__":
    ...
    species_array = ["Escherichia_coli", "Clostridioides_difficile", "Enterococcus_faecium",
                    "Pseudomonas_aeruginosa", "Salmonella_enterica","Shigella_flexneri",
                    "Shigella_sonnei", "Staphylococcus_aureus", "Streptococcus_agalactiae",
                    "Streptococcus_suis", "Acinetobacter_baumannii", "Corynebacterium_diphtheriae",
    ...
    # Load indicators
    indicators_df = pd.read_csv("Results/Normalized_indicators.csv", index_col=[0], header=[0])

    #Import forcast bounds (ensures percentages remain 0-100, no unrealistic negative indicators)
    IndicatorBounds=pd.read_csv('Results/Forecasting Global/IndicatorForecastBounds.csv',index_col=[0], header=[0])
    ...
    for specie in species_array:
        ...
        for count, class_name in enumerate(linear_regression_df.keys()):
            ...
            for k in range(len(RegressionParameters)):
                ...
                file_indicator = 'Results/Forecasting Global/MonteCarloIndicatorForecasts/IndicatorForecast_montecarlosimulations_'+specie+'_'+class_name+'_Indicator_'+ str(IndicatorBounds.loc[indicator,"Code"])+'.csv'
                ...
                for i in range(len(features_rate_df)):
                    country = features_rate_df.loc[i, "Country Code"]
                    year = features_rate_df.loc[i, "Collection Year"]
                ...
                country_array = features_rate_df["Country Code"]
                country_array = np.delete(country_array,idx_nans)
                year_array = features_rate_df["Collection Year"]
                ...
                RatesForecastSims=pd.DataFrame(data=Rateslist, columns=years_indicator_forecast) #years_total
                RatesForecastSims.to_csv('Results/Forecasting Global/MonteCarloFeaturesForecasts/FeatureForecast_'+specie+'_'+class_name+'_LinearRegressionRow_'+str(k)+'.csv')
