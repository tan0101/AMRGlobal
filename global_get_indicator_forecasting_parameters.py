...
if __name__ == "__main__":
    ...
    species_array = [sys.argv[1]]

    if not os.path.exists("Results/Forecasting/IndicatorForecasts"):
        os.makedirs("Results/Forecasting/IndicatorForecasts")

    if not os.path.exists("ResultsForecasting/IndicatorParameters"):
        os.makedirs("Results/Forecasting/IndicatorParameters")

    # Load indicators
    indicators_df = pd.read_csv("Results/Correlations/Normalized_indicators.csv", index_col=[0], header=[0])

    #Import forcast bounds (ensures percentages remain 0-100, no unrealistic negative indicators)
    IndicatorBounds=pd.read_csv('Results/Forecasting/IndicatorForecastBounds.csv',index_col=[0], header=[0])

    for specie in species_array:
        print(specie)
        feature_rate_df = pd.read_excel("Results/Correlations/"+specie+".xlsx", sheet_name=None)

        writer_curve = pd.ExcelWriter("Results/Forecasting/IndicatorParameters/Indicator_CurveParameters_"+specie+".xlsx", engine='xlsxwriter')

        update_progress(0)
        for count, class_name in enumerate(feature_rate_df.keys()):
            anti_df = pd.read_excel("Results/Correlations/"+specie+".xlsx", sheet_name=class_name, header=[0], index_col=[0])
            ...
            for indicator in indicators_df.columns[2:]:
                ...
                for i in range(len(anti_df)):
                    country = anti_df.loc[i, "Country Code"]
                    year = anti_df.loc[i, "Collection Year"]

                    idx_country = np.where(indicators_df["Country Code"] == country)[0]
                    idx_year = np.where(indicators_df["Year"] == year)[0]

                    idx_intersect = np.intersect1d(idx_country, idx_year)

                    indicator_array[i] = indicators_df.loc[idx_intersect,indicator]
                ...
                country_array = anti_df["Country Code"]
                country_array = np.delete(country_array,idx_nans)
                year_array = anti_df["Collection Year"]
                ...
                if len(Indicator_forecast) > 0:
                    Indicator_forecast.to_csv('Results_mixedmodel/Forecasting Global/IndicatorForecasts/'+specie+'_'+class_name+"_indicator_"+str(IndicatorBounds.loc[indicator,"Code"])+'.csv')  
