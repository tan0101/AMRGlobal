...
if __name__ == "__main__":
    ...
    if not os.path.exists("Results/Forecasting/MonteCarloIndicatorForecasts"):
        os.makedirs("Results/Forecasting/MonteCarloIndicatorForecasts")
    ...
    for specie in species_array:
        print(specie)
        curve_dataframe = pd.read_excel("Results/Forecasting/IndicatorParameters/Indicator_CurveParameters_"+specie+".xlsx", sheet_name=None)


        for count, class_name in enumerate(curve_dataframe.keys()):
            print(class_name)
            curve_df = pd.read_excel("Results/Forecasting/IndicatorParameters/Indicator_CurveParameters_"+specie+".xlsx", sheet_name=class_name, header=[0], index_col=[0])
            ...
            for count_indicator, indicator in enumerate(indicators_df.columns[2:]):
                ...
                for row in curve_df.index:
                    ...
                    if curve_df.loc[row]['Result'] == 'Linear curve fitted':
                        ...
                        df.to_csv('Results/Forecasting/MonteCarloIndicatorForecasts/IndicatorForecast_montecarlosimulations_'+specie+'_'+class_name+'_Indicator_'+str(IndicatorBounds.loc[indicator,"Code"])+ '.csv')
