...
if __name__ == "__main__":
    ...
    for count, filename in enumerate(files_array):
        ...
        specie = split_filename[1]+"_"+split_filename[2]
        ...
        trend_df.loc[count,"Countries"] = RegressionParameters.loc[row,"Countries"]
        trend_df.loc[count,"Num Countries"] = RegressionParameters.loc[row,"Num Countries"]
        ...
        if len(np.unique(df["data"])) > 1:
            ...
            for k in ["1%", "5%", "10%"]:
                ...
                if trend_df.loc[count,"Feature Result "+str(np.round(100-int(k[:-1]),0))+"%"] == "Not stationary" or trend_df.loc[count,"Feature Result "+str(np.round(100-int(k[:-1]),0))+"%"] == "Difference stationary" or trend_df.loc[count,"Feature Result "+str(np.round(100-int(k[:-1]),0))+"%"] == "Trend stationary":
                    if df.loc[df.index[0], "data"] > df.loc[df.index[-1], "data"]:
                        trend_df.loc[count,"Feature Trend "+str(np.round(100-int(k[:-1]),0))+"%"] = "Decreasing"
                    elif df.loc[df.index[0], "data"] < df.loc[df.index[-1], "data"]:
                        trend_df.loc[count,"Feature Trend "+str(np.round(100-int(k[:-1]),0))+"%"] = "Increasing"
                    elif df.loc[df.index[0], "data"] == df.loc[df.index[-1], "data"]:
                        trend_df.loc[count,"Feature Trend "+str(np.round(100-int(k[:-1]),0))+"%"] = "Check!"
                else:
        ...
        timeseries_df = pd.read_csv("Results/Forecasting Global/MonteCarloIndicatorForecasts/IndicatorForecast_montecarlosimulations_"+specie+"_"+class_name+"_Indicator_"+str(IndicatorBounds.loc[indicator,"Code"])+".csv",header=[0], index_col=[0])
            ...
            for k in ["1%", "5%", "10%"]:
                ...
                if trend_df.loc[count,"Indicator Result "+str(np.round(100-int(k[:-1]),0))+"%"] == "Not stationary" or trend_df.loc[count,"Indicator Result "+str(np.round(100-int(k[:-1]),0))+"%"] == "Difference stationary" or trend_df.loc[count,"Indicator Result "+str(np.round(100-int(k[:-1]),0))+"%"] == "Trend stationary":
                    if df.loc[df.index[0], "data"] > df.loc[df.index[-1], "data"]:
                        ...
                        trend_df.loc[count,"Indicator Trend "+str(np.round(100-int(k[:-1]),0))+"%"] = "Check!"
                else:
